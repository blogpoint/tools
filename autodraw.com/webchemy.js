/*
	...
	Copyright (C) 2014 bitbof (http://bitbof.com)
...
var Webchemy = (function () {
    ...
    UI.showOpaquePopup = (function () {
        ...
        return function (p) {
            ...
            if (!popDiv) {
                ...
                popDiv.appendChild(popDiv.bgDiv);
                popDiv.overlay = document.createElement("div");
                LIB.css(popDiv.overlay, {
                    position: "absolute",
                    left: "0",
                    top: "0",
                    width: "100%",
            ...
            updateHeight();
            popDiv.appendChild(p.div);
            popDiv.appendChild(popDiv.overlay);
            popDiv.ontouchmove = function () {
                return false;
            };
            p.target.appendChild(popDiv);
            setTimeout(function () {
                ...
                setTimeout(function () {
                    popDiv.removeChild(popDiv.overlay);
                }, 310);
            }, 20);
    ...
    UI.showInfoPage = function (parent, callback) {
        ...
        helpPage = (function () {
            ...
            pageElements[3].push(UI.createDiv({
                innerHTML: 'More Resources<br/><br/><a href="http://www.youtube.com/playlist?list=PLEwyLb78MLYn0Tj1b6F37uwOCOAanTD7X" target="_blank">Youtube Videos about Webchemy</a></br></br><br/><a href="http://webchemy.org/gallery/" target="_blank">User Gallery</a>',
		...
		var discoverPage = (function () {
            var div = document.createElement("div");
            //div.innerHTML = "<center><div style=\"padding-top:10px;font-family:'Lucida Sans Unicode', 'Lucida Grande', sans-serif;\">" + ' Webchemy is also available on the Play Store:</div></center>';
			//div.innerHTML += '<br/><center><a target="_blank" href="https://play.google.com/store/apps/details?id=com.bitbof.webchemy">Webchemy Android App</a></center><br/>';
            div.innerHTML += "<center><div style=\"padding-top:10px;font-family:'Lucida Sans Unicode', 'Lucida Grande', sans-serif;\">" + ' More drawing web-apps by bitbof:</div></center>';
			div.innerHTML += '<br/><a target="_blank" href="http://kleki.com">Kleki</a> - Painterly oriented drawing tool.';
			div.innerHTML += '</br></br><a target="_blank" href="http://bitbof.com/doodler">Doodler</a> - Sketching practice tool.';
            LIB.css(div, {
                fontSize: (boxSize / 3.0) + "px",
                color: UI.theme.labelColor,
                display: "none",
	...
	*/
    function WebchemyBar(p) {
        ...
        if (false) { //window.chrome) { //webintents not supported anymore
	...
	*/
    function WebchemyCanvasView(p) {
        ...
        height = p.height;
        zoom = 1;
		(function() { //reset fit on init
			var canvasSize = canvas.getSize();
			//canvasSize.width /= 2;
			//canvasSize.height /= 2;
			var fit = LIB.fitInto(width, height, canvasSize.width, canvasSize.height);
			zoom = fit.width / canvasSize.width;
		})();
        ...
        function setCursor(c) {
            ...
            if (cursor === CURSOR_PICKER && cursorPickerIm) {
                div.style.cursor = "url(" + cursorPickerIm + ") 0 15, pointer";
            }
        }
        ...
        function onKeys(val) {
            ...
            if (val.down) {
				...
				if (unicode === keys.KEY_S && keys.ctrlPressed) {
                    val.preventDefault();
                    callback({
                        save: true
                    });
				...
				if (unicode === keys.KEY_X && !keys.spacePressed && !keys.ctrlPressed && !keys.altPressed) {
					callback({
						switchColor: true
					});
                }
                if (unicode === keys.KEY_SPACE) {
					...
					if (mode[mode.length - 1] === MODE_HAND_SPACE) {
						mode.pop();
					}
        ...
        function update() {
			...
			if(translate.x === 0 && translate.y === 0) {
				var canvasSize = canvas.getSize();
				var fit = LIB.fitInto(width, height, canvasSize.width, canvasSize.height);
				if(zoom === fit.width / canvasSize.width) {
					isFit = true;
				}
			}
            callback({
		...
		//resets the View so the image is centered and at 100%
        function reset() {
            if (!zoomTransition) {
                zoomTransition = true;
                LIB.css(canvasWrapper, {
		...
		/*
		Resets the view, that the canvas fits within the view.
		Previously it centered and zoomed to 100%. But that would make it awkward to use with arbitrarily high resolution pictures.
		*/
        function resetFit() {
            if (!zoomTransition) {
                zoomTransition = true;
	...
	*/
    function WebchemyBrush(p) {
		...
		var inputSpacing = 7; //distance the input has to traverse before line gets updated
        ...
        this.goLine = function (p) {
			...
			if (action.input.length < 2 || (Math.random() > 0.05 && oldlen === action.input.length && action.input.length > 80)) {
                return false;
            }
			if(mode === MODE_PULL && oldlen === action.input.length) {
				return false;
			}
            ...
            if (mode === MODE_PULL) {
                if (Math.random() > 0.8 || action.pulls.length === 0) {
                ...
                for (i = 0; i < action.input.length - 1; i += 1) {
                    ...
                    } else if (i === action.input.length - 3) { //this thing is a hack to make stroke lines look nicer
                        pathString += dat.x + "," + dat.y + " L " + dat.c1.x + "," + dat.c1.y + " " + dat.c2.x + "," + dat.c2.y;
                    } else {
        ...
        this.drawActionOnContext = function (p) {
			...
			function lineTo(x1, y1, x2, y2, x3, y3) {
            ...
            function drawPath() {
                ...
                for (i = 0; i < a.input.length - 2; i += 1) {
					...
					if (a.splat) {
						lineTo(dat.c1.x, dat.c1.y, dat.c2.x, dat.c2.y, dat.c3.x, dat.c3.y);
					} else {
						if(i === a.input.length - 3) { //hack to make stroke lines look nicer
							lineTo(dat.c1.x, dat.c1.y, dat.c2.x, dat.c2.y, dat.c3.x, dat.c3.y);
						} else {
	...
	*/
    ...
    //NOT PUBLIC
    UI.introPopup = function (p, p2) {
		var large, boxPx, boxSize, width, height, imLinkPre, parent, div, page, bg, moreButton, closeButton, closeFunc;
        large = p2;
        boxPx = UI.theme.halfBoxSize / 22;
        boxSize = UI.theme.halfBoxSize * 2;
        width = 7 * 2 * UI.theme.halfBoxSize;
        height = boxPx * 200;
        imLinkPre = "intro";
        if (large) {
            width = boxPx * 700;
		...
		function createSmall() {
			...
			LIB.css(bg, {
				width: "100%",
				height: "100%",
				backgroundImage: "url(" + imLinkPre + "01.jpg)",
			...
			div.onclick = function () {
				...
				page += 1;
				if (page < 4) {
					bg.style.backgroundImage = "url(" + imLinkPre + "0" + page + ".jpg)";
				}
			};
			...
			moreButton.innerHTML = '<a href="about">Learn More About Webchemy</a>';
		}
		
		function createLarge() {
			var pages = [];
			pages[0] = document.createElement("div");
			
			var splashVersion = 0;
			var introImage = "introL01.jpg?v=3";
			var roll = Math.random();
			if(roll > 0.33) {
				splashVersion = 1;
				introImage = "introL01_02.jpg?v=1";
			}
			if(Math.random() > 0.66) {
				splashVersion = 1;
				introImage = "introL01_03.jpg?v=1";
			}
			...
			pages[1] = document.createElement("div");
			LIB.css(pages[1], {
				width: "100%",
				height: "100%",
				backgroundImage: "url(introL02.jpg)",
				backgroundSize: "100% 100%",
				backgroundColor: "#777",
				borderRadius: (boxSize / 4) + "px",
				position: "absolute",
			...
			pages[2] = document.createElement("div");
			LIB.css(pages[2], {
				width: "100%",
				height: "100%",
				backgroundImage: "url(introL03.jpg)",
				backgroundSize: "100% 100%",
				borderRadius: (boxSize / 4) + "px",
				position: "absolute",
				top: 0,
			...
			sharingDiv.innerHTML = '<div id="WebchemyAppSharingDiv"><div id="share_fbook"><iframe id="iframefbook" src="http://www.facebook.com/plugins/like.php?href=http%3A%2F%2Fwebchemy.org&amp;layout=button_count&amp;show_faces=false&amp;width=55&amp;action=like&amp;colorscheme=light&amp;locale=en_US" style="width:90px; height:20px;" allowtransparency="true" frameborder="0" scrolling="no"></iframe></div>	<div id="share_twitter">		<iframe id="iframetwitter" src="http://platform.twitter.com/widgets/tweet_button.html?url=http%3A%2F%2Fwebchemy.org&amp;text=Create%20idea%20sparking%20sketches%20with%20Webchemy!&amp;count=count" style="width:115px; height:20px;" allowtransparency="true" frameborder="0" scrolling="no"></iframe>	</div>	<div id="share_gplus">		<iframe id="iframeplusone" src="https://plusone.google.com/u/0/_/+1/fastbutton?url=http%3A%2F%2Fwebchemy.org&amp;size=medium&amp;lang=de" scrolling="no" frameborder="0" style="border:none; overflow:hidden; width:74px; height:20px;" align="left"></iframe>	</div></div>';
			...
			pages[0].onclick = function() {
				...
				div.removeChild(artButton);
				setTimeout(function() {
					div.removeChild(pages[0]);
				}, 510);
			};
			pages[1].onclick = function() {
				...
				setTimeout(function() {
					div.removeChild(pages[1]);
				}, 510);
			};
			div.appendChild(pages[2]);
			div.appendChild(pages[1]);
			div.appendChild(pages[0]);
			div.appendChild(sharingDiv);
			...
			LIB.css(moreButton, {
				...
				color: "#fff",
				cursor: "pointer",
				boxShadow: "0 5px 5px rgba(0,0,0,0.5), inset 0 3px 3px rgba(255,255,255,0.5)"
			});
			moreButton.innerHTML = "<a href=\"http://webchemy.org/about\">Learn More</a>";
			moreButton.className = "WebchemyAppShadedPopupBG testo";
			setTimeout(function() {
				LIB.addCssRule(".testo a", "border-radius: 20px; display: block;height: 100%, background: none; background-image: none; background-scale: 0; margin: 0; padding: 0; color: #fff; text-shadow: none;");
				LIB.addCssRule(".testo a:hover", "color: #fff; background-color: rgba(0,0,0,0.1)");
			}, 100);
			pages[2].appendChild(moreButton);
			var bitbofButton = document.createElement("div");
			bitbofButton.innerHTML = '<a target="_blank" href="http://bitbof.com">bitbof 2014</a>';
			LIB.css(bitbofButton, {
				position: "absolute",
				top: "100%",
				left: "100%",
				width: "110px",
			...
			div.appendChild(bitbofButton);
			
			var artButton = document.createElement("div");
			artButton.innerHTML = '<a target="_blank" href="http://chilkat.deviantart.com/">art by Chilkat</a>';
			LIB.css(artButton, {
				position: "absolute",
				top: "94%",
				left: "100%",
			...
			if(splashVersion == 0) {
				div.appendChild(artButton);
			}
			var showPlayStoreLink = true;
			if(window.chrome && Math.random() > 0.4) {
				if (!chrome.app.isInstalled) {
					showPlayStoreLink = false;
					var chromeAppButton = document.createElement("div");
					chromeAppButton.innerHTML = '<span style="font-size: 18px">+</span> ADD TO CHROME';
					chromeAppButton.title = "Add Webchemy as an app to your Chrome browser.";
					LIB.css(chromeAppButton, {
						position: "absolute",
						top: "100%",
						left: "6px",
						marginTop: "-40px",
						textAlign: "center",
						backgroundColor: "#4e7dd7", 
						color: "#fff",
						padding: "5px",
						paddingLeft: "10px",
						paddingRight: "10px",
					...
					div.appendChild(chromeAppButton);
					chromeAppButton.onclick = function() {
						_gaq.push(['_trackEvent', 'Webchemy', 'install', 'chrome app']);
						chrome.webstore.install();
						div.removeChild(chromeAppButton);
					};
				}
			}
			if(showPlayStoreLink) {
				/*var androidButton = document.createElement("a");
				androidButton.href = "https://play.google.com/store/apps/details?id=com.bitbof.webchemy";
				LIB.css(androidButton, {
					position: "absolute",
					display: "block",
					bottom: "7px",
					left: "7px",
					width: "161px",
					height: "54px",
					backgroundImage: "url(android_app_button.png)",
					paddingRight: 0
				});
				div.appendChild(androidButton);*/
			}
			//hi south korea
			/*
			var helloDrawing = new Image();
			helloDrawing.src = "welcome.png";
			helloDrawing.onload = function () {
				setTimeout(function() {
					LIB.css(helloDrawing, {
		...
		LIB.addCssRule("#WebchemyAppSharingDiv", "opacity: 0.8; width: 100%; height: 100%; border-top-left-radius: 20px; border-top-right-radius: 20px; -webkit-transition: opacity 0.2s ease-in-out; -moz-transition: opacity 0.2s ease-in-out;");
			LIB.addCssRule("#WebchemyAppSharingDiv:hover", "opacity: 1;");
	...
	*/
    function WebchemyApp(p) {
		...
		function saveDrawing() {
			var finalim, canvas, im;
			var ismobile=navigator.userAgent.match(/(iPad)|(iPhone)|(iPod)|(android)|(webOS)/i);
			var is_firefox = navigator.userAgent.toLowerCase().indexOf('firefox') > -1;
			if ((is_firefox && !ismobile) || (window.chrome && !global.isAndroid)) {
        ...
        bar = new WebchemyBar({
            parent: webchemyDiv,
            callback: function (val) {
                var BB, bb, data, intent, onSuccess, onError;
                if (val.color) {
                    tools.updateBrush({
                        color: val.color
                    });
                ...
                if (val.exportSVG) {
					var bb = new Blob([tools.getSVGString()], {type : 'image/svg'});
                    window.saveAs(bb, "webchemy.svg");
                }
                if (val.exportPNG) {
					saveDrawing();
                }
                ...
                if (val.intentEdit) {
                    ...
                    intent = new WebKitIntent("http://webintents.org/edit", "image/png", data);
                    onSuccess = function () {};
                    onError = function () {};
		...
		LIB.addCssRule("#share_fbook", "position: absolute; top: 10px; left: 20px;");
		LIB.addCssRule("#share_twitter", "position: absolute; top: 10px; left: 110px;");
		LIB.addCssRule("#share_gplus", "position: absolute; top: 10px; left: 215px;");
		...
		if(!global.isAndroid) {
			if(!localStorage.getItem("wcReturningUser")) {
				UI.introPopup(webchemyDiv, (width > 700 && height > 400));
				localStorage.setItem("wcReturningUser", true);
			}
		} else {
			if(Math.random() > 1) {
				//android app button
				(function() {
					...
					LIB.css(message, {
						...
						paddingTop: "10px",
						boxSizing: "border-box",
						lineHeight: (size / 2 - 10) + "px",
						pointerEvents: "none"
					});
					message.innerHTML = "<b>Webchemy for Android</b><br/>on Google Play";
					
					
					var installContainer = document.createElement("div");
					LIB.css(installContainer, {
						cssFloat: "left",
						height: "100%",
						boxSizing: "border-box",
						padding: "10px"
					...
					installBtn.href = "https://play.google.com/store/apps/details?id=com.bitbof.webchemy";
					...
					centerContainer.appendChild(closeBtn);
					centerContainer.appendChild(message);
					centerContainer.appendChild(installContainer);
					adiv.appendChild(centerContainer);
